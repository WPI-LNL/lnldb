    ...
    def __iter__(self):
        ...
        if self.field.cache_choices and False:  # disable unintelligent caching
            if self.field.choice_cache is None:
                self.field.choice_cache = [
                    (self.field.group_label(group), [
                        self.choice(ch) for ch in choices]) for group, choices in groupby(
            ...
            for group, choices in groupby(
                ...
                # yield (self.field.group_label, [self.choice(ch) for ch in
                # choices])


# was going to use this for fancier grouped dropdowns. Leaving it here,
# but no calls to it
