...
class Migration(DataMigration):
    def forwards(self, orm):
        ...
        for org in orm.Organization.objects.all():
            account = orm.Fund()
            account.fund = org.fund
            account.organization = org.organization
            account.account = org.account
            account.name = "%s Default" % org.name
            account.last_updated = datetime.date.today()
            account.save()
            org.accounts.add(account)
            org.save()
        for event in orm.Event.objects.all():
            if event.billing_org and event.billing_org.accounts.count() > 0:
                event.billing_fund = event.billing_org.accounts.first()
            elif event.org.count() > 0 and event.org.first().accounts.count() > 0:
                event.billing_fund = event.org.first().accounts.first()
            event.save()

        # add a default one to fall back on.
        account = orm.Fund()
        account.fund = 0
