...
class Migration(SchemaMigration):
    def forwards(self, orm):
        # Removing unique constraint on 'Hours', fields ['event', 'user']
        db.delete_unique(u'events_hours', ['event_id', 'user_id'])

        # Adding field 'Hours.service'
        db.add_column(u'events_hours', 'service',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='hours', null=True,
                                                                            to=orm['events.Service']),
                      keep_default=False)

        # Adding unique constraint on 'Hours', fields ['event', 'service', 'user']
        db.create_unique(u'events_hours', ['event_id', 'service_id', 'user_id'])

    def backwards(self, orm):
        # Removing unique constraint on 'Hours', fields ['event', 'service', 'user']
        db.delete_unique(u'events_hours', ['event_id', 'service_id', 'user_id'])

        # Deleting field 'Hours.service'
        db.delete_column(u'events_hours', 'service_id')

        # Adding unique constraint on 'Hours', fields ['event', 'user']
        db.create_unique(u'events_hours', ['event_id', 'user_id'])
