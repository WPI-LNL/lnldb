...
class Migration(SchemaMigration):
    def forwards(self, orm):
        # Adding M2M table for field services on 'Extra'
        db.create_table('events_extra_services', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('extra', models.ForeignKey(orm['events.extra'], null=False)),
            ('service', models.ForeignKey(orm['events.service'], null=False))
        ))
        db.create_unique('events_extra_services', ['extra_id', 'service_id'])

        # Removing M2M table for field extra on 'Service'
        db.delete_table('events_service_extra')

    def backwards(self, orm):
        # Removing M2M table for field services on 'Extra'
        db.delete_table('events_extra_services')
