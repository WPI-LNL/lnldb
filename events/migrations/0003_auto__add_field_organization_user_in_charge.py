...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Organization.user_in_charge'
        db.add_column('events_organization', 'user_in_charge',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, related_name='orgowner', to=orm['auth.User']),
                      keep_default=False)

        # Adding M2M table for field associated_users on 'Organization'
        db.create_table('events_organization_associated_users', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('organization', models.ForeignKey(orm['events.organization'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))
        ))
        db.create_unique('events_organization_associated_users', ['organization_id', 'user_id'])

        # Adding M2M table for field associated_orgs on 'Organization'
        db.create_table('events_organization_associated_orgs', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('from_organization', models.ForeignKey(orm['events.organization'], null=False)),
            ('to_organization', models.ForeignKey(orm['events.organization'], null=False))
        ))
        db.create_unique('events_organization_associated_orgs', ['from_organization_id', 'to_organization_id'])


    def backwards(self, orm):
        # Deleting field 'Organization.user_in_charge'
        db.delete_column('events_organization', 'user_in_charge_id')

        # Removing M2M table for field associated_users on 'Organization'
        db.delete_table('events_organization_associated_users')

        # Removing M2M table for field associated_orgs on 'Organization'
        db.delete_table('events_organization_associated_orgs')
    ...
    models = {
        ...
        'events.organization': {
            ...
            'associated_orgs': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'associated_orgs_rel_+'", 'to': "orm['events.Organization']"}),
