...
class BaseEvent(PolymorphicModel):
    ...
    def status(self):
        if self.cancelled:
            ...
            return "Closed"
        elif self.approved and self.datetime_setup_complete > datetime.datetime.now(timezone.utc) and not self.reviewed:
            return "Approved"
    ...
    @property
    def over(self):
        return self.datetime_end < datetime.datetime.now(timezone.utc)
...
class Event(BaseEvent):
    ...
    def contact_addr(self):
        if self.contact:
            ...
            # if self.datetime_setup_complete < datetime.datetime.now(timezone.utc):
...
class OrganizationTransfer(models.Model):
    ...
    def is_expired(self):
        if self.completed:
            return True
        elif datetime.datetime.now(timezone.utc) > self.expiry:
            return True
        return False
